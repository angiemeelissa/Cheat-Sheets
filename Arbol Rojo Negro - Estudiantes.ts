class Student {
    public name: string;
    public age: number;
    public id: number; //CARNET
    public career: string;

    constructor(name: string, age: number, id: number, career: string) {
        this.name = name;
        this.age = age;
        this.id = id; //CARNET
        this.career = career;
...
class RBTree {
    ...
    constructor() {
        this.leaf = new NodeRBT(new Student("", 0, 0, ""), true);
        this.root = this.leaf;
    }
    ...
    public delete(id: number): void {
        ...
        if (!nodeToDelete || nodeToDelete === this.leaf) { 
            console.log(`Estudiante con Carnet ${id} NO Enontrado en el Arbol`);
            return;
        }
    ...
    private RecorridoPostOrden(nodo: NodeRBT = this.root): void {
        if (nodo !== this.leaf) {
            this.RecorridoPostOrden(nodo.getLeftChild());
            this.RecorridoPostOrden(nodo.getRightChild());
            console.log(`${nodo.getData().toString()} (${nodo.getColor()})`);
        }
    }

    //RECORRIDO PRE-ORDEN
    private RecorridoPreOrden(nodo: NodeRBT = this.root): void {
        if (nodo !== this.leaf) {
            console.log(`${nodo.getData().toString()} (${nodo.getColor()})`);
            this.RecorridoPreOrden(nodo.getLeftChild());
            this.RecorridoPreOrden(nodo.getRightChild());
        }
    }
    ...
    public postOrder(): void {
        this.RecorridoPostOrden(this.root);
    }
...
function addStudent(name: string, age: number, id: number, career: string) {
    const student = new Student(name, age, id, career);
    studentTree.insert(student);
    console.log(`Estudiante ${name} agregado al árbol.`);
}
...
addStudent("Angie Santiago", 19, 1555123, "Ingenieria");
addStudent("Armando Champet", 21, 1662523, "Ingenieria");
addStudent("Taylor Swift", 34, 1989013, "Musica");
